...
import parmap
from bsds.bsds.thin import *
from bsds.bsds.correspond_pixels import *
...
def evaluate_boundaries(predicted_boundaries, gt_boundaries,
    ...
    for i_t, thresh in enumerate(progress(list(thresholds))):
        ...
        if apply_thinning:
            predicted_boundaries_bin = binary_thin(
...
def single_evaluate_boundaries(_thresholds, predicted_boundaries, gt_boundaries, max_dist):
    sum_p = np.zeros(_thresholds.shape)
    count_p = np.zeros(_thresholds.shape)
    sum_r = np.zeros(_thresholds.shape)
    count_r = np.zeros(_thresholds.shape)

    for i_t, thresh in enumerate(list(_thresholds)):
        predicted_boundaries_bin = predicted_boundaries >= thresh
...
def multi_evaluate_boundaries(predicted_boundaries, gt_boundaries,
                        thresholds=99, max_dist=0.0075, apply_thinning=True,
                        progress=None):
    if progress is None:
        progress = lambda x, *args, **kwargs: x
    ...
    splited_thresholds =  np.array_split(thresholds, num_cores)
    
    predicted_boundaries = [predicted_boundaries] * len(splited_thresholds)
    gt_boundaries = [gt_boundaries] * len(splited_thresholds)
    max_dist = [max_dist] * len(splited_thresholds)

    tasks = zip(splited_thresholds, predicted_boundaries, gt_boundaries, max_dist)
    result = parmap.starmap(single_evaluate_boundaries, tasks, pm_processes=num_cores)

    count_r = np.concatenate([res[0] for res in result]).ravel()
    sum_r = np.concatenate([res[1] for res in result]).ravel()
    count_p = np.concatenate([res[2] for res in result]).ravel()
    sum_p = np.concatenate([res[3] for res in result]).ravel()
